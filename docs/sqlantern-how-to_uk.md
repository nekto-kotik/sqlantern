...
- Використовуйте функцію `Підтримувати це з'єднання` - натисніть ![](https://sqlantern.com/images/icon_keep_alive.png) в будь-якій панелі із таблицями. Це _зазвичай_ подовжує сесію на сервері (в PHP доволі складна логіка закінчення часу сесії), але не може повністю запобігти її закінченню (особливо якщо таб браузера із SQLantern _викинутий_ з пам'яті).
